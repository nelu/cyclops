...
const RULES: AlertUpdateRule[] = [{
  ...
  check: (alert, update) => !(update.outcome && !alert.notes),
}, {
  ...
  message: 'Alert status cannot be changed to New after it has been ' +
    'resolved. Change the status to Busy, then unassign to change it back ' +
    'to New.',
  check: (alert, update) => !(update.status === 'NEW' && alert.status === 'DONE'),
}, {
  message: 'Outcome must be selected before changing the status to Done.',
  check: (alert, update) => !(update.status === 'DONE' && !alert.outcome),
}, {
  message: 'Analysis cannot be changed after alert has been resolved.',
  check: (alert, update) => !(update.notes && alert.status === 'DONE'),
}];
